        ...
        if (lock.tryLock() || Instant.now().isAfter(tokenExpiry) ) {
            try {
                ...
                if (response.statusCode() == 400) {
                    LOG.error("Failed to fetch access token, status code: {} body: {}", response.statusCode(), response.body());
                    throw new RuntimeException("Failed to fetch access token, status code: " + response.statusCode() + " body: " + response.body());
                }
                if (response.statusCode() != 200) {
                    LOG.error("Failed to fetch access token, is credentials correct/not locked?, status code: {}", response.statusCode());
